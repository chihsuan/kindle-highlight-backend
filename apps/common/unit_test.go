	...
	r.POST("/login", func(c *gin.Context) {
		...
		if err := c.Bind(&json); err == nil {
			if json.Username == "wangzitian0" && json.Password == "0123456789" {
				...
				c.JSON(http.StatusUnauthorized, errors.New("wrong username or password"))
			}
		} else {
...
func TestNewError(t *testing.T) {
	...
	commenError := db.Find(NotExist{heheda: "heheda"}).Error
	...
	assert.Equal(map[string]interface{}(map[string]interface{}{"database": "no such table: not_exists"}),
		commenError, "commenError should have right error info")
}
